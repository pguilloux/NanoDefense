				...
				for(int i=0; i<agents.size(); i++)
				{
					...
					if(agents.get(i).getLife()<=0 && agents.get(i).getMove())
					{
						if(agents.get(i).getMod(2))
							for(int k=0; k<4; k++)
								agents.add(new Agent(agents.get(i).getProprio(),players.get(0),1,agents.get(i).getx(),agents.get(i).gety()+k,agents.get(i).getPath(),agents.get(i).getZoneStart(),agents.get(i).getZoneStop(),map));
						agents.get(i).setMove(false);
						if(agents.get(i).getProprio()==2)
						players.get(0).win(10);
